---
title: "SmarkForm Forms — Agent Knowledge"
layout: default
permalink: /resources/AGENTS/SmarkForm-Forms
nav_exclude: true
---
...
into SmarkForm internals see
[SmarkForm Usage — Agent Knowledge]({{ "/resources/AGENTS/SmarkForm-Usage" | relative_url }});
for adding examples to the Jekyll documentation see
`AGENTS/Documentation-Examples.md` in the repository.
...
{: .hint}
> See [CDN Resources]({{ "/resources/CDN" | relative_url }}) for a full list
> of available CDN links, including older versions.
...
Download `SmarkForm.esm.js` or `SmarkForm.umd.js` from the
[Download]({{ "/resources/download" | relative_url }}) section, place it
alongside your HTML, and reference it with a relative path:
...
> `data-smark` on the same element — SmarkForm raises a `VALUE_CONFLICT` error.
...
```
Using SmarkForm (CDN ESM: {{ smarkform_esm_cdn_latest }}),
...
```
Using SmarkForm loaded via CDN (UMD: {{ smarkform_umd_cdn_latest }}),
...
- [ ] Buttons inside cloned list items that target a sub-list use
      ...
      (see [SmarkForm Usage]({{ "/resources/AGENTS/SmarkForm-Usage" | relative_url }}) — "Buttons inside vs. outside a list").
